...
// 🔥 FIXED WAKE WORD (FUZZY, RELIABLE)
function isWakeWord(text) {
  const t = text.toLowerCase();
  return t.includes("sara"); // catches sara / sarah / hey sara / okay sara
}
...
async function handleFinalTranscript(text) {
  ...
  if (isSleeping) {
    if (!isWakeWord(text)) {
      console.log("💤 Sleeping, ignored:", text);
      return;
    }

    // 🌅 WAKE UP
    isSleeping = false;
    const wakeReply = "Yes? I’m here.";
    console.log(`🤖 SaRA: ${wakeReply}`);
    await speak(wakeReply);
    return;
  }
  ...
  try {
    ...
    if (isGoodbyeIntent(text)) {
      isSleeping = true;
      const byeReply = "Alright. Talk to you later.";
      console.log(`🤖 SaRA: ${byeReply}`);
      await speak(byeReply);
      return;
    }
